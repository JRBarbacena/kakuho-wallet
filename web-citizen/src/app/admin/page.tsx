                                ...
                                <table className="data-table">
                                    ...
                                    <tbody>
                                        {registrations.slice(0, 5).map((reg, i) => (
                                            <tr key={i}><td>{reg.public_name}</td><td><code>{reg.registry_cid?.slice(0, 24) || 'Pending CID'}...</code></td><td><span className="text-emerald-400 text-[10px] font-bold">ACTIVE</span></td></tr>
                                        ))}
                                    </tbody>
                                </table>
                            </div>
                    ...
                    {activeView === 'view-management' && (
                        <div className="glass-panel p-10 max-w-2xl mx-auto space-y-8">
                            ...
                            <div className="space-y-4">
                                ...
                                <input type="text" placeholder="Paste Hash here..." value={scannedHash} onChange={(e) => setScannedHash(e.target.value)} className="w-full bg-black/30 border border-white/10 p-4 rounded-lg text-white font-mono text-xs outline-none focus:border-orange-500" />
                            </div>
                            <div className="space-y-4">
                                <label className="text-[10px] font-black uppercase text-slate-500">Admin Authorization</label>
                                <input type="password" placeholder="Enter Admin Password" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} className="w-full bg-black/30 border border-white/10 p-4 rounded-lg text-white font-mono text-xs outline-none focus:border-orange-500" />
                            </div>
